...
user_tag_config['ebound_header_tag']['mobile']['adsCode'] = '';
			...
			<div class="tittle_head_w3ls">
				<h3 class="tittle">Viewing Applicant</h3>
			</div>
		<!---728x90--->
<script src='../../../../../../../publisher.eboundservices.com/dynamicAds/dynamicScript.js'></script>
<div style='margin: 0 auto;text-align: center;margin-top: 5px;'><script>
					...
					<div class="login-form-rec">
						<form action="ViewApplication.jsp" >
							<table class = "table-striped table-bordered table-condensed" >
								...
								<%	/* Getting Category from Login Page --> Admin Category */
									String cat = (String) session.getAttribute("Category");
									System.out.println("View Category = "+cat);
									/* Getting the View Button Id of each scheme Name */
									String vid = request.getParameter("id");
									System.out.println("VID = "+vid);
									session.setAttribute("id",vid);
									/* Declaration of Variables */
									String fname="";String lname="";String mail="";
									try
									{
										Class.forName("com.mysql.jdbc.Driver");
										Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/social","root","");
										Statement stmt = con.createStatement();
										/* Getting the firstname, lastname and email from registration based on category and role */
										String sqla = "SELECT FirstName,LastName,Email FROM registration where Category = '"+cat+"' and Role = '"+"User"+"'";
										System.out.println("A = "+sqla);
										ResultSet res = stmt.executeQuery(sqla);
										while(res.next())
										{
										%>
											<tr>
													<%	/* Saving the values to a variable name */
										...
										<% 
											...
											/* Query for displaying the user details based on a criteria */
											String sqlb = "SELECT SchemeId,UserId,PayStatus,RejectStatus FROM applyuser where Category = '"+cat+"' AND UserId = '"+mail+"' AND SchemeId = '"+vid+"' AND  Status = '"+"1"+"'";
											System.out.println("B ="+sqlb);
											ResultSet rest = stmts.executeQuery(sqlb);
											while(rest.next())
											{
											%>
												<%
													String em = rest.getString("UserId");
													System.out.println(em);
													String Sid = rest.getString("SchemeId");
													System.out.println("Sid = "+Sid);
													int Pstatus = rest.getInt("PayStatus");
													System.out.println("Pstatus = "+Pstatus);
													int Rstatus = rest.getInt("RejectStatus");
													System.out.println("Rstatus = "+Rstatus);
												%>
												<!-- Displaying all the Contents -->  <!-- From First Query -->
												<td><%=res.getString("FirstName") %>
													<%=res.getString("LastName") %>
												</td>
												<!-- Second Query -->
												<td><%=rest.getString("UserId") %></td>
												<%
												if(Pstatus==1 && em.equals(mail) && Sid.equals(vid))
												{
												%>
													<td><button class = "btn btn-success" name = "approve" value = "Approved" disabled>Approved</button></td>
												<%		
												}
												else if(Rstatus==1 && em.equals(mail) && Sid.equals(vid))
												{
												%>
													<td><button class = "btn btn-danger" name = "reject" value = "Rejected" disabled>Rejected</button></td>
												<%		
												}
												else
												{
												%>
														<!-- Passing email as a parameter to view application -->
													<td><a href="ViewApplication.jsp?mail=<%=rest.getString("UserId")%>"><input type = "button" name = "viewapp" class = "btn btn-primary" value = "&nbsp View &nbsp"></a></td>
